...
def test_policy_get_with_multiple_conditions(fake_config: ControlPlaneConfig) -> None:
    research_policy = PolicyModel(
        ...
        document_policy=DocumentPolicy(mik_top_k=2, max_top_k=8),
    )
    ...
    for case in test_cases:
        ...
        if case["expect_found"]:
            ...
            if case["policy_name"] == "research_policy":
                assert result.document_policy.mik_top_k == 2, case["name"]
                assert result.document_policy.max_top_k == 8, case["name"]
            continue
...
@pytest.mark.parametrize(
    ...
    [
        (
            DocumentPolicy(mik_top_k=0, max_top_k=5),
            "document_policy.mik_top_k must be greater than 0",
        ),
        (
            DocumentPolicy(mik_top_k=3, max_top_k=0),
            "document_policy.max_top_k must be greater than 0",
        ),
        (
            DocumentPolicy(mik_top_k=6, max_top_k=5),
            "document_policy.mik_top_k must be less than or equal to document_policy.max_top_k",
        ),
    ],
)
